...
class RatingReliabilityEngine
{
    ...
    public function calculate(array $facts, array $configuration = [], ?string $asOf = null): array
    {
        ...
        $partners = max(0, (int) ($facts['partner_count'] ?? 0));
        ...
        $components = [
            ...
            'partner_diversity' => min(100, ($partners / max(1, (int) ($configuration['partner_target'] ?? 6))) * 100),
            'competition_diversity' => min(100, ($competitions / max(1, (int) ($configuration['competition_target'] ?? 4))) * 100),
        ];
        $weights = $configuration['reliability_weights'] ?? [
            'volume' => .30,
            'verification' => .25,
            'recency' => .20,
            'opponent_diversity' => .15,
            'partner_diversity' => .10,
            'competition_diversity' => .10,
        ];
        $score = 0.0;
    ...
    {
        $facts = ['rated_match_count' => 0, 'verified_match_count' => 0, 'opponent_count' => 0, 'partner_count' => 0, 'competition_type_count' => 0, 'last_rated_match_at' => null];
        if ($this->db->tableExists('rating_transactions')) {
            ...
            foreach ($rows as $row) {
                ...
                foreach (($metadata['partner_ids'] ?? []) as $id) $partners[(int) $id] = true;
            ...
            $facts['partner_count'] = count($partners);
    ...
    {
        $facts = ['rated_match_count' => 0, 'verified_match_count' => 0, 'opponent_count' => 0, 'partner_count' => 0, 'competition_type_count' => 0, 'last_rated_match_at' => $lastMatchAt];
        $opponents = []; $partners = []; $types = [];
        if ($this->db->tableExists('rating_transactions')) foreach ($this->db->table('rating_transactions')->where('tenant_id', $tenantId)->where('player_id', $playerId)->where('discipline_id', $disciplineId)->where('status', 'applied')->whereIn('transaction_type', ['impact', 'replacement'])->get()->getResult() as $row) {
            ...
            $metadata = is_string($row->metadata ?? null) ? (json_decode($row->metadata, true) ?: []) : (array) ($row->metadata ?? []);
            foreach ((array) ($metadata['opponent_ids'] ?? []) as $id) $opponents[(int) $id] = true;
            foreach ((array) ($metadata['partner_ids'] ?? []) as $id) $partners[(int) $id] = true;
            if (! empty($metadata['match_type'])) $types[$metadata['match_type']] = true;
            $facts['last_rated_match_at'] = $row->created_at;
        }
        foreach ($opponentIds as $id) if ((int) $id > 0 && (int) $id !== $playerId) $opponents[(int) $id] = true;
        foreach ($partnerIds as $id) if ((int) $id > 0 && (int) $id !== $playerId) $partners[(int) $id] = true;
        if ($matchType) $types[$matchType] = true;
        $facts['opponent_count'] = count($opponents);
        $facts['partner_count'] = count($partners);
        $facts['competition_type_count'] = count($types);
        $facts['last_rated_match_at'] = $lastMatchAt ?: $facts['last_rated_match_at'];
        return $facts;
    }
}

                    ...
                    <div class="bracket-roster-grid">
                        <?php foreach ($categoryAthletes as $index => $athlete): ?>
                            ...
                            <div class="bracket-athlete">
                                ...
                                <div class="bracket-athlete-info"><div class="bracket-athlete-name"><?php if (! empty($athlete->player_id)): ?><a href="<?= base_url('admin/players/profile/' . (int) $athlete->player_id) ?>" target="_blank"><?= esc($athleteName) ?></a><?php else: ?><?= esc($athleteName) ?><?php endif; ?><?php if ($partnerName !== ''): ?> / <?php if (! empty($athlete->partner_player_id)): ?><a href="<?= base_url('admin/players/profile/' . (int) $athlete->partner_player_id) ?>" target="_blank"><?= esc($partnerName) ?></a><?php else: ?><?= esc($partnerName) ?><?php endif; ?><?php endif; ?></div><div class="bracket-athlete-meta"><?= esc($athlete->player_code ?? 'Chưa có mã') ?><?= isset($athlete->rating_score) && $athlete->rating_score !== null ? ' · Rating ' . esc((string) $athlete->rating_score) : '' ?></div></div>
                    ...
                    <div class="bracket-round">
                        ...
                        <div class="bracket-round-items">
                            ...
                            <article class="bracket-match" data-bracket-match>
                                ...
                                <div class="bracket-match-info"><span><?= esc($match->scheduled_date ?? '-') ?> <?= $match->start_time ? esc(substr($match->start_time, 0, 5)) : '' ?></span><span><?= $match->court_id ? 'Sân ' . (int) $match->court_id : 'Chưa xếp sân' ?><?= isset($match->team_a_id) ? ' · A:' . (int) $match->team_a_id : '' ?><?= isset($match->team_b_id) ? ' / B:' . (int) $match->team_b_id : '' ?></span></div>
